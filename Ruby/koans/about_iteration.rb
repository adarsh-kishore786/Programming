...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    # if `arr` is an iterable,
    # arr.inject(c) { |total, item| f }
    # does this:
    # total starts off with an initial value of `c`, the `item` is the current
    # item in array we are iterating over. The new `total` becomes
    # `total = f`
    # Here `f` is any general function (can be `nil` too). It is usually
    # dependent on `total` and `item` (but not necessarily)
  end
