...
public class Vector
{
    ...
    public Vector(double mag, double[] theta)
    {
        ...
        assert DoubleComp.equals(sum, 1.0) : "The angles are mathematically invalid.";
    ...
    protected static String invalidOp(Vector v1, Vector v2)
    {
        return "The operation is invalid for dimension " + v1.dim() + 
                    " and " + v2.dim();
    }
    ...
    {
        assert i < comp.length : "Invalid index accessed";
        return comp[i];
    }
    ...
    {
        assert v1.dim() == v2.dim() : Vector.invalidOp(v1, v2);
        int l = v1.dim();

        double[] diff = new double[l];
        for (int i = 0; i < l; i++)
            diff[i] = v1.getComp(i) - v2.getComp(i);
    ...
    {
        int l = v1.dim();
        assert l == v2.dim() : Vector.invalidOp(v1, v2);
